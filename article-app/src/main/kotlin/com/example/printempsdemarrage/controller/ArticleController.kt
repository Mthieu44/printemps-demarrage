...
class ArticleController(val articleRepository: ArticleRepository) {
    ...
    fun findOne(@PathVariable id: String): ResponseEntity<ArticleDTO> {
        val article = articleRepository.get(id.toInt())
    ...
    fun update(@PathVariable id: String, @RequestBody @Valid article: ArticleDTO): ResponseEntity<Any> {
        var idConvertis = 0
        try {
            idConvertis = id.toInt()
        } catch (e: Exception) {
            ResponseEntity.badRequest().body("Erreur de conversion de String vers entier")
        }
        return if (idConvertis != article.id) {
    ...
    fun delete(@PathVariable id: String): ResponseEntity<Any> {
        ...
        val deleted = articleRepository.delete(idConvertis)
